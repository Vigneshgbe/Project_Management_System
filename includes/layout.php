...
.logo-mark{
  ...
  font-family:var(--font-display);font-weight:700;font-size:16px;color:#fff;flex-shrink:0
}
...
.nav-item{
  ...
  border-radius:var(--radius-sm);color:var(--text2);font-size:13.5px;font-weight:500;
  transition:background .15s,color .15s;margin-bottom:1px;white-space:nowrap
}
...
.modal-header{
  ...
  padding:18px 20px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--bg2)
}
...
<div id="main">
  <header id="header">
    ...
    <?php renderHeaderActions($activePage, $user); ?>
  </header>
  <main id="content">
<?php
} // end renderLayout

function renderHeaderActions(string $page, array $user): void {
    $now = date('D, M j');
    echo '<span style="font-size:12px;color:var(--text3)">'.$now.'</span>';
}
...
function toggleDropdown(id){
  ...
  document.querySelectorAll('.dropdown-menu').forEach(x=>{if(x.id!==id)x.classList.remove('open')});
