...
.lg-email{font-size:12px;color:#6366f1;word-break:break-all}
.lg-addr{max-width:200px;font-size:12px;color:var(--text3)}
...
.lg-call-btn{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;background:#10b981;color:#fff;border-radius:50%;text-decoration:none;font-size:12px;border:none;cursor:pointer;flex-shrink:0;transition:opacity .15s}
...
.lg-imp-btn{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;background:var(--orange);color:#fff;border-radius:50%;border:none;cursor:pointer;font-size:11px;flex-shrink:0;transition:opacity .15s}
...
.lg-email-badge:hover{opacity:.8}
...
<div id="lg-no-api-banner" style="display:none;background:rgba(249,115,22,.08);border:1px solid rgba(249,115,22,.3);border-radius:var(--radius-lg);padding:14px 18px;margin-bottom:16px;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
  <div style="display:flex;align-items:center;gap:10px">
    ...
    <div>
      <div style="font-size:13px;font-weight:700;color:var(--orange)">Google Places API key not configured</div>
      <div style="font-size:12px;color:var(--text2)">Takes 2 minutes to setup. Free trial credits cover thousands of searches.</div>
    </div>
  </div>
...
<div class="lg-search-card">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">
    <div>
      ...
      <div style="font-size:12px;color:var(--text3);margin-top:2px">Fetches business name, owner, phone, email, address, website & rating from Google Maps</div>
    </div>
    <?php if (isAdmin()): ?>
  ...
  <div id="lg-cost-preview" style="margin-bottom:12px;font-size:12.5px;color:var(--text3)">
    Each search costs approximately <strong>$0.032</strong> (text search) + <strong>$0.003 × leads</strong> (details lookup). Monthly limit protects you automatically.
  </div>
...
<div class="lg-manage-section" id="lg-manage-section" style="display:none">
  ...
  <div class="lg-filter-bar">
    ...
    <select id="filter-website" class="lg-filter-select" onchange="filterStoredLeads()">
      <option value="">All</option><option value="1">Has Website</option><option value="0">No Website</option>
    </select>
...
function updateCostPreview() {
    ...
    document.getElementById('lg-cost-preview').innerHTML=
        ...
        '<span style="color:#6366f1">✉ Emails scraped free from business websites.</span>';
}
...
function loadStats() {
    ...
    .then(function(d){
        if (!d.ok) return;
        lgConfigured=d.api_set; lgQuota=d.quota||300; lgBudget=d.budget||15; lgUsedLeads=d.used||0;
        ...
        fill.style.stroke=pct>95?'#ef4444':pct>80?'#f59e0b':'#fff';
        ...
        document.getElementById('lg-cost-fill').style.background=cpct>80?'#ef4444':cpct>60?'#f59e0b':'#10b981';
    ...
    .catch(function(e){ console.error('loadStats error:',e); });
}
...
function renderRecent(data) {
    ...
    el.innerHTML=data.map(function(r,i){
        ...
        return '<div class="lg-act" onclick="loadSearchHistory('+r.id+')">'
...
function doSearch() {
    ...
    if (lgUsedLeads+cnt>lgQuota) {
        toast('⛔ Monthly quota limit reached! '+lgUsedLeads+'/'+lgQuota+' used.','error'); return;
    }
    ...
    document.getElementById('lg-load-sub').textContent='Fetching phone, email, website info for each result...';
...
function loadAllStoredLeads(page) {
    ...
    .then(function(d){
        if (!d.ok) {
            toast(d.error||'Failed to load','error');
            if (tbody) tbody.innerHTML='<tr><td colspan="12" style="text-align:center;padding:36px"><div style="font-size:28px;margin-bottom:8px">⚠️</div><div style="color:var(--text)">Error: '+esc(d.error||'Failed')+'</div></td></tr>';
            return;
        }
        ...
        if (countEl) countEl.textContent='Total: '+d.total+' leads stored';
    })
    .catch(function(e){
        toast('Network error: '+e.message,'error');
        if (tbody) tbody.innerHTML='<tr><td colspan="12" style="text-align:center;padding:36px"><div style="color:var(--text3)">Network Error: '+esc(e.message)+'</div></td></tr>';
    });
}
...
function renderPagination(total,currentPage,perPage) {
    ...
    if (startPage>1) { html+='<button onclick="loadAllStoredLeads(1)" class="btn btn-ghost btn-sm">1</button>'; if (startPage>2) html+='<span style="padding:5px 8px;color:var(--text3)">...</span>'; }
    ...
    if (endPage<totalPages) { if (endPage<totalPages-1) html+='<span style="padding:5px 8px;color:var(--text3)">...</span>'; html+='<button onclick="loadAllStoredLeads('+totalPages+')" class="btn btn-ghost btn-sm">'+totalPages+'</button>'; }
...
function populateFilters(locations,industries) {
    ...
    if (locSelect&&locations&&locations.length) {
        var cv=locSelect.value;
        locSelect.innerHTML='<option value="">All Locations</option>'+locations.map(function(loc){return '<option value="'+esc(loc)+'">'+esc(loc)+'</option>';}).join('');
        locSelect.value=cv;
    }
    if (indSelect&&industries&&industries.length) {
        var cv=indSelect.value;
        indSelect.innerHTML='<option value="">All Industries</option>'+industries.map(function(ind){return '<option value="'+esc(ind)+'">'+esc(ind)+'</option>';}).join('');
        indSelect.value=cv;
    }
}
...
function resetFilters() {
    ['filter-search','filter-location','filter-industry','filter-website','filter-imported'].forEach(function(id){
...
function toggleSelectAll(checkbox) {
    document.querySelectorAll('.lead-select').forEach(function(cb){cb.checked=checkbox.checked;});
    updateBulkDeleteButton();
}
function updateSelectAllState() {
    var selectAll=document.getElementById('select-all-leads');
    var all=document.querySelectorAll('.lead-select');
    var checked=document.querySelectorAll('.lead-select:checked').length;
    if (!selectAll) return;
    if (all.length===0) { selectAll.checked=false; selectAll.indeterminate=false; }
    else if (checked===0) { selectAll.checked=false; selectAll.indeterminate=false; }
...
function bulkDeleteSelected() {
    var selected=Array.from(document.querySelectorAll('.lead-select:checked')).map(function(cb){return cb.dataset.id;});
    if (!selected.length) { toast('No leads selected','info'); return; }
    ...
    var fd=new FormData(); fd.append('action','bulk_delete'); fd.append('ids',selected.join(','));
    fetch('lead_generator_api.php',{method:'POST',body:fd})
    .then(function(r){return r.json();})
    .then(function(d){
    ...
    .catch(function(e){toast('Network error: '+e.message,'error');btn.disabled=false;updateBulkDeleteButton();});
}
...
function saveSettings(){
    ...
    .then(function(r){return r.json();})
    .then(function(d){
        if(d.ok){toast('Settings saved!','success');lgConfigured=true;document.getElementById('lg-no-api-banner').style.display='none';document.getElementById('cfg-key').value='';loadStats();}
