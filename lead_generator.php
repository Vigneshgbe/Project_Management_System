...
.lg-ring-src{font-size:10px;color:rgba(255,255,255,.6);margin-top:4px}
/* ── CARDS ── */
...
/* ── RECENT ── */
.lg-act{display:flex;align-items:flex-start;gap:8px;padding:7px 4px;border-bottom:1px solid var(--border);cursor:pointer;transition:background .12s;border-radius:4px}
...
/* ── QUOTA MANAGEMENT ── */
.lg-quota-panel{margin-top:20px;padding-top:18px;border-top:2px solid var(--border)}
...
<div class="lg-top" id="lg-top-row">
  <div class="lg-ring-card">
    ...
    <div class="lg-ring-wrap">
      <svg class="lg-ring-svg" viewBox="0 0 100 100" width="100" height="100">
        ...
        <circle class="lg-ring-fill" id="lg-ring" cx="50" cy="50" r="42" stroke-dasharray="264" stroke-dashoffset="264"/>
      </svg>
      <div class="lg-ring-inner">
        <div class="lg-ring-pct" id="lg-pct">0%</div>
        <div class="lg-ring-sub" id="lg-sub">0 / 0</div>
      </div>
    </div>
    <div class="lg-ring-src" id="lg-quota-src"></div>
  </div>
  ...
  <div class="lg-cost-card">
    ...
    <div style="margin-top:10px;font-size:11.5px;color:var(--text3);line-height:1.7">
      ...
      <div style="margin-top:4px;font-size:10.5px">Google $200 free/month</div>
    </div>
  </div>
</div>
...
<div class="lg-settings" id="lg-settings-panel">
  ...
  <div id="stab-content-api">
    <div style="background:var(--bg2);border-radius:var(--radius-sm);padding:14px;margin-bottom:16px">
      <div style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:10px">How to get your Google Places API key:</div>
  ...
  <div id="stab-content-quota" style="display:none">
    <div class="lg-quota-panel" style="padding-top:0;border-top:none">
      <div style="font-size:13px;font-weight:700;margin-bottom:4px">👥 Lead Quota by Role</div>
      <div style="font-size:12px;color:var(--text3);margin-bottom:14px">Set monthly lead limits per role. User-level overrides take priority over role defaults. Set <strong>0</strong> to block a user/role entirely.</div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        ...
        <div style="font-size:11px;color:var(--text3)">Leave blank = inherit role default &nbsp;|&nbsp; 0 = blocked</div>
      </div>
...
<div class="lg-search-card">
  ...
  <!-- Quota status banner (shown to non-admin, and to all when quota is low) -->
  ...
  <div style="margin-bottom:14px">
    <div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:7px">🎯 Quick Targets <span style="text-transform:none;letter-spacing:0;font-weight:400;color:var(--text3)">(click to fill)</span></div>
  ...
  <div id="lg-cost-preview" style="margin-bottom:12px;font-size:12.5px;color:var(--text3)">
    Each search: <strong>$0.032</strong> (text search) + <strong>$0.003 × leads</strong> (detail lookups). At 5 leads: ~$0.047.
  </div>
  ...
  <div class="lg-search-row">
    ...
    <div>
      ...
      <input type="text" id="lg-industry" class="lg-input" placeholder="e.g. Restaurant, Law firm">
    </div>
    <div>
...
<div class="lg-manage-section" id="lg-manage-section" style="display:none">
  ...
  <div style="overflow-x:auto">
    <table class="lg-tbl">
      ...
      <tbody id="lg-stored-tbody"><tr><td colspan="12" style="text-align:center;padding:24px;color:var(--text3)">Loading...</td></tr></tbody>
    </table>
  </div>
...
<div id="lg-results-section" style="display:none" class="lg-results-card">
  <div class="lg-results-head">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <button onclick="importAll()" class="btn btn-sm" id="lg-imp-all-btn" style="background:var(--orange);color:#fff;border:none">⬇ Import All to CRM</button>
  ...
  <div style="overflow-x:auto">
    <table class="lg-tbl">
      <thead>
        <tr><th>Score</th><th>#</th><th>Business Name / Owner</th><th>Phone</th><th>Email</th><th>Address</th><th>Website</th><th>Rating</th><th>Actions</th></tr>
      </thead>
...
var lgUsed=0, lgRemaining=0, lgBlocked=false, lgQuotaSrc='role';
...
function updateCostPreview() {
    ...
    document.getElementById('lg-cost-preview').innerHTML=
        'This search: <strong style="color:var(--orange)">$'+cost+'</strong> (1 text search + '+cnt+' detail lookups). '
        +(lgRemaining>0?'<span style="color:var(--text3)">You have '+lgRemaining+' leads remaining this month.</span>':'');
}
...
function loadStats() {
    ...
    .then(function(d){
        ...
        document.getElementById('lg-blocked-banner').style.display=d.blocked?'block':'none';
        ...
        document.getElementById('lg-quota-src').textContent=
            lgQuotaSrc==='user'?'(custom quota)':lgQuotaSrc==='global'?'(admin quota)':'(role default)';
        ...
        // Cost meter — uses global cost so admin can see total system usage
...
function renderUserQuotaBanner(d) {
    ...
    if (d.blocked) {
        b.className='lg-quota-banner blocked'; b.style.display='flex';
        b.innerHTML='🚫 <strong>You have no lead quota.</strong> Contact admin to get access.'; return;
    }
    var pct=d.quota>0?Math.round(d.used/d.quota*100):0;
    var rem=d.remaining||0;
    if (rem<=0) {
        b.className='lg-quota-banner danger'; b.style.display='flex';
        b.innerHTML='⛔ <strong>Monthly quota exhausted.</strong> You\'ve used all '+d.quota+' leads. Resets next month or ask admin to increase your limit.';
    } else if (pct>=70) {
        b.className='lg-quota-banner warn'; b.style.display='flex';
        b.innerHTML='⚠ <strong>'+rem+' leads remaining</strong> this month ('+pct+'% used). Use wisely!';
    } else {
        b.className='lg-quota-banner ok'; b.style.display='flex';
        b.innerHTML='✓ <strong>'+rem+' leads remaining</strong> this month &nbsp;·&nbsp; Quota: '+d.used+' / '+d.quota
            +(d.quota_source==='user'?' <span style="font-size:11px;opacity:.7">(custom)</span>':'');
    }
}
...
function renderRecent(data) {
    ...
    el.innerHTML=data.map(function(r,i){
        ...
        return '<div class="lg-act" onclick="loadSearchHistory('+r.id+')" title="Reload these leads">'
            +'<div class="lg-act-dot" style="background:'+colors[i%colors.length]+'"></div>'
            +'<div style="flex:1;min-width:0"><div style="font-size:12.5px;color:var(--text2);font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(r.result_count)+' leads · '+esc(r.industry)+' in '+esc(r.location)+'</div>'
            +'<div style="font-size:11px;color:var(--text3)">$'+cost+' · '+fmtAgo(r.created_at)+'</div></div></div>';
    }).join('');
}
...
function doSearch() {
    ...
    if (cnt>lgRemaining) { toast('You only have '+lgRemaining+' leads remaining. Reduce count to '+lgRemaining+' or less.','error'); document.getElementById('lg-count').value=lgRemaining; return; }
    ...
    var fd=new FormData();
    fd.append('action','search'); fd.append('location',loc); fd.append('industry',ind);
    fd.append('count',cnt); fd.append('search_mode',lgSearchMode||'no_website');
    ...
    .then(function(d){
        ...
        if (!d.leads||!d.leads.length) { toast(d.message||'No results found.','info'); return; }
        renderResults(d.leads,ind,loc);
        if (d.used!==undefined) {
            lgUsed=d.used; lgRemaining=d.remaining||0;
    ...
    .catch(function(e){
        btn.disabled=false; btn.textContent='Generate';
        loading.style.display='none'; toast('Network error','error'); console.error(e);
    });
}
...
function renderResults(leads,ind,loc) {
    ...
    document.getElementById('lg-res-label').textContent='('+leads.length+' results — '+ind+', '+loc+(nowebCount?' — '+nowebCount+' without website':'')+')';
    ...
    document.getElementById('lg-tbody').innerHTML=leads.map(function(l,i){
        ...
        var phone_btn=l.phone?'<a href="tel:'+esc(l.phone)+'" class="lg-call-btn" title="Call '+esc(l.phone)+'">📞</a>':'';
        var email_btn=l.email?'<a href="mailto:'+esc(l.email)+'" class="lg-mail-btn" title="Email '+esc(l.email)+'">✉</a>':'';
        var web_badge=hasWeb?'<a href="'+esc(l.website)+'" target="_blank" class="lg-web-yes">✅ Yes ↗</a>':'<span class="lg-web-no">🔥 No</span>';
        var imp_btn=l.imported?'<span class="lg-imp-done">✓ In CRM</span>':'<button class="lg-imp-btn" onclick="impOne('+l.id+',this)" title="Import to CRM">⬇</button>';
        ...
        var scoreBadge='<div style="text-align:center"><div style="font-size:16px;font-weight:900;color:'+sc+'">'+score+'</div>'
            +'<div style="font-size:10px;font-weight:700;background:'+scoreBg+';color:'+sc+';padding:1px 5px;border-radius:99px;white-space:nowrap">'+(score>=70?'🔥 HOT':score>=40?'👍 Good':'·')+'</div></div>';
        return '<tr id="lgr-'+l.id+'" style="'+(hasWeb?'':'background:rgba(249,115,22,.03)')+'">'
            +'<td>'+scoreBadge+'</td><td style="color:var(--text3);font-size:12px">'+(i+1)+'</td>'
            +'<td><div class="lg-name">'+esc(l.name)+'</div>'+(l.owner_name?'<div class="lg-owner">👤 '+esc(l.owner_name)+'</div>':'')+(l.why?'<div class="lg-opp-reason" title="'+esc(l.why)+'">💡 '+esc(l.why.length>60?l.why.slice(0,57)+'...':l.why)+'</div>':'')+'</td>'
            +'<td>'+(l.phone?'<span class="lg-phone">'+esc(l.phone)+'</span>':'<span class="lg-no-data">—</span>')+'</td>'
            +'<td>'+(l.email?'<a href="mailto:'+esc(l.email)+'" class="lg-email-badge">'+esc(l.email)+'</a>':'<span class="lg-no-data">—</span>')+'</td>'
            +'<td><div class="lg-addr" title="'+esc(l.address)+'">'+esc(l.address||'—')+'</div></td>'
            +'<td>'+web_badge+'</td>'
            +'<td class="lg-rating">'+(l.rating?'⭐ '+parseFloat(l.rating).toFixed(1):'—')+(l.ratings_total?'<div style="font-size:10px;color:var(--text3)">'+l.ratings_total+' reviews</div>':'')+'</td>'
...
function importAll() {
    ...
    var btn=document.getElementById('lg-imp-all-btn'); btn.disabled=true; btn.textContent='Importing...';
    ...
    fetch('lead_generator_api.php',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(d){
        ...
        if(d.ok){toast(d.imported+' leads added to CRM! 🎉','success');pending.forEach(function(id){var b=document.querySelector('#lgr-'+id+' .lg-imp-btn');if(b)b.replaceWith(Object.assign(document.createElement('span'),{className:'lg-imp-done',textContent:'✓ In CRM'}));});}
...
function setMode(mode){
    lgSearchMode=mode; document.getElementById('lg-search-mode').value=mode;
    ['no_website','high_value','all'].forEach(function(m){var el=document.getElementById('lm-'+m);if(el) el.classList.toggle('mode-active',m===mode);});
}
function setIndustry(btn,val){
    var el=document.getElementById('lg-industry'); if(el){el.value=val;el.focus();}
    document.querySelectorAll('.chip-active').forEach(function(b){b.classList.remove('chip-active');b.style.borderColor='var(--border)';b.style.color='var(--text2)';});
    if(btn){btn.classList.add('chip-active');btn.style.borderColor='var(--orange)';btn.style.color='var(--orange)';}
}
function toggleSettings(){var p=document.getElementById('lg-settings-panel');if(!p)return;p.classList.toggle('open');}
...
function renderQuotaUsers(users,globalQuota) {
    ...
    tbody.innerHTML=users.map(function(u){
        var used=parseInt(u.used_leads)||0;
        var eq=parseInt(u.effective_quota)||0;
        ...
        return '<tr>'
            ...
            +'<td><input type="number" name="user_quota['+u.id+']" class="qu-input" value="'+overrideVal
                +'" min="-1" max="5000" placeholder="inherit" title="Leave blank=role default, 0=block, number=custom limit"></td>'
            +'</tr>';
    }).join('');
}
...
function saveQuotaConfig() {
    ...
    fetch('lead_generator_api.php',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(d){
        if(msg){msg.style.display='block';msg.style.color=d.ok?'#10b981':'#ef4444';msg.textContent=d.ok?'✓ '+d.message:(d.error||'Save failed');setTimeout(function(){msg.style.display='none';},3000);}
...
function saveSettings(){
    ...
    var fd=new FormData(); fd.append('action','save_settings'); fd.append('google_key',key); fd.append('quota',quota); fd.append('budget',budget);
    fetch('lead_generator_api.php',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(d){
        if(d.ok){toast('Settings saved! Click "🔌 Test" to verify.','success');lgConfigured=true;document.getElementById('lg-no-api-banner').style.display='none';document.getElementById('cfg-key').value='';loadStats();}
        else toast(d.error||'Save failed','error');
    });
}
function testKey(){
    var btn=document.getElementById('cfg-test-btn'); var res=document.getElementById('cfg-test-result');
    var key=document.getElementById('cfg-key')?.value.trim()||'';
    btn.disabled=true; btn.textContent='Testing...'; if(res) res.style.display='none';
    var fd=new FormData(); fd.append('action','test_key'); if(key) fd.append('google_key',key);
    fetch('lead_generator_api.php',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(d){
        btn.disabled=false; btn.textContent='🔌 Test';
        if(res){res.style.display='block';res.style.background=d.ok?'rgba(16,185,129,.08)':'rgba(239,68,68,.06)';res.style.border=d.ok?'1px solid rgba(16,185,129,.25)':'1px solid rgba(239,68,68,.2)';res.style.color=d.ok?'#10b981':'#ef4444';res.textContent=d.ok?d.message:d.error;}
...
function loadSearchHistory(usageId){
    var loading=document.getElementById('lg-loading'); var results=document.getElementById('lg-results-section');
    loading.style.display='block'; if(results) results.style.display='none';
...
function toggleManageSection(){
    ...
    if(sec.style.display==='none'||sec.style.display===''){
        sec.style.display='block'; loadAllStoredLeads(1); if(btn) btn.textContent='✕ Close Stored Leads';
    } else { sec.style.display='none'; if(btn) btn.textContent='📚 View All Stored Leads ('+lgTotalStored+')'; }
}
...
function loadAllStoredLeads(page){
    ...
    fetch(url).then(function(r){return r.json();}).then(function(d){
        ...
        var c=document.getElementById('lg-manage-count'); if(c) c.textContent='Total: '+d.total+' leads stored';
        lgTotalStored=d.total; var ts=document.getElementById('lg-total-stored'); if(ts) ts.textContent=d.total;
...
function renderStoredLeadsTable(leads){
    ...
    if(!leads||!leads.length){tbody.innerHTML='<tr><td colspan="12" style="text-align:center;padding:36px"><div style="font-size:28px;margin-bottom:8px">📭</div><div style="color:var(--text3)">No leads found</div></td></tr>';return;}
    tbody.innerHTML=leads.map(function(l){
        ...
        var web_badge=hasWeb
            ?(l.website?'<a href="'+esc(l.website)+'" target="_blank" class="lg-web-yes" style="font-size:10px">✅ Yes ↗</a>':'<span class="lg-web-yes" style="font-size:10px">✅ Yes</span>')
            :'<span class="lg-web-no" style="font-size:10px">🔥 No</span>';
        var status=l.imported?'<span class="lg-imp-done" style="font-size:10px">✓ Imported</span>':'<span style="font-size:10px;color:var(--text3)">Not imported</span>';
        return '<tr style="'+(hasWeb?'':'background:rgba(249,115,22,.02)')+'">'
            +'<td><input type="checkbox" class="lead-select" data-id="'+l.id+'"></td>'
            +'<td>'+scoreBadge+'</td>'
            +'<td><div style="font-weight:700;font-size:13px;color:var(--text)">'+esc(l.name)+'</div>'+(l.owner_name?'<div style="font-size:11px;color:var(--text3)">👤 '+esc(l.owner_name)+'</div>':'')+'</td>'
            +'<td style="font-size:12px">'+(l.phone?esc(l.phone):'<span style="color:var(--text3)">—</span>')+'</td>'
            +'<td>'+(l.email?'<a href="mailto:'+esc(l.email)+'" style="font-size:11px;color:var(--orange)">'+esc(l.email)+'</a>':'<span style="color:var(--text3);font-size:11px">—</span>')+'</td>'
            +'<td style="font-size:12px;color:var(--text3)">'+esc(l.location||'—')+'</td>'
            +'<td style="font-size:12px;color:var(--text3)">'+esc(l.industry||'—')+'</td>'
            +'<td>'+web_badge+'</td>'
            +'<td style="font-size:12px">'+(l.rating?'⭐ '+parseFloat(l.rating).toFixed(1):'—')+(l.ratings_total?'<div style="font-size:10px;color:var(--text3)">'+l.ratings_total+'</div>':'')+'</td>'
            +'<td style="font-size:11px;color:var(--text3)">'+fmtDate(l.created_at)+'</td>'
            +'<td>'+status+'</td>'
            +'<td><div style="display:flex;gap:4px">'+(l.imported?'':'<button onclick="impOne('+l.id+',this)" class="lg-imp-btn" style="width:24px;height:24px;font-size:10px" title="Import">⬇</button>')
                +'<button onclick="deleteSingleLead('+l.id+')" class="btn btn-ghost btn-sm btn-icon" style="width:24px;height:24px;padding:4px;font-size:11px" title="Delete">🗑</button></div></td>'
            +'</tr>';
    }).join('');
...
function renderPagination(total,currentPage,perPage){
    ...
    var info=document.getElementById('lg-pagination-info'); var btns=document.getElementById('lg-pagination-btns');
    if(!info||!btns) return;
    info.textContent='Showing '+(((currentPage-1)*perPage)+1)+'–'+Math.min(currentPage*perPage,total)+' of '+total;
    ...
    for(var i=s;i<=e;i++){if(i===currentPage)html+='<button class="btn btn-sm" style="background:var(--orange);color:#fff;border:none">'+i+'</button>';else html+='<button onclick="loadAllStoredLeads('+i+')" class="btn btn-ghost btn-sm">'+i+'</button>';}
...
function populateFilters(locations,industries){
    var locSel=document.getElementById('filter-location'); var indSel=document.getElementById('filter-industry');
...
function resetFilters(){['filter-search','filter-location','filter-industry','filter-website','filter-imported'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});loadAllStoredLeads(1);}
function toggleSelectAll(cb){document.querySelectorAll('.lead-select').forEach(function(c){c.checked=cb.checked;});updateBulkDeleteButton();}
function updateSelectAllState(){var sa=document.getElementById('select-all-leads');var all=document.querySelectorAll('.lead-select');var chk=document.querySelectorAll('.lead-select:checked').length;if(!sa)return;sa.checked=chk>0&&chk===all.length;sa.indeterminate=chk>0&&chk<all.length;updateBulkDeleteButton();}
function updateBulkDeleteButton(){var btn=document.getElementById('lg-bulk-delete-btn');if(!btn)return;var sel=document.querySelectorAll('.lead-select:checked');btn.disabled=sel.length===0;btn.textContent='🗑 Delete Selected'+(sel.length>0?' ('+sel.length+')':'');}
...
function bulkDeleteSelected(){
    ...
    var btn=document.getElementById('lg-bulk-delete-btn'); btn.disabled=true; btn.textContent='Deleting...';
