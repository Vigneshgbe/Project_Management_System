...
function lgSet(mysqli $db, string $k, string $v, int $uid): void {
    ...
    $db->query("INSERT INTO lead_gen_settings (setting_key,setting_val,updated_by)
                VALUES ('$ke','$ve',$uid)
                ON DUPLICATE KEY UPDATE setting_val='$ve',updated_by=$uid");
}
...
function lgMonthUsed(mysqli $db, int $uid): array {
    $r = @$db->query("SELECT COALESCE(SUM(result_count),0) leads,
                             COALESCE(SUM(api_calls_used),0) calls,
                             COALESCE(SUM(estimated_cost),0) cost
                      FROM lead_gen_usage
                      WHERE user_id=$uid
                        AND YEAR(created_at)=YEAR(NOW())
                        AND MONTH(created_at)=MONTH(NOW())");
    return $r ? ($r->fetch_assoc() ?: ['leads'=>0,'calls'=>0,'cost'=>0])
              : ['leads'=>0,'calls'=>0,'cost'=>0];
}
...
function lgIsRealWebsite(string $url): bool {
    if (!$url) return false;
    $bad=['facebook.com','fb.com','instagram.com','twitter.com','x.com','g.co','goo.gl',
          'maps.google.com','google.com/maps','yelp.com','tripadvisor.com','justdial.com',
          'indiamart.com','youtube.com','linkedin.com','whatsapp.com','t.me'];
    $low=strtolower($url);
    foreach ($bad as $b) { if (strpos($low,$b)!==false) return false; }
    return true;
}
function lgEnsureColumns(mysqli $db): void {
    ...
    @$db->query("UPDATE lead_gen_results SET has_website=0 WHERE website IS NOT NULL AND website!='' AND (
        website LIKE '%facebook.com%' OR website LIKE '%fb.com%' OR website LIKE '%instagram.com%' OR
        website LIKE '%twitter.com%' OR website LIKE '%x.com%' OR website LIKE '%g.co%' OR
        website LIKE '%goo.gl%' OR website LIKE '%maps.google%' OR website LIKE '%youtube.com%' OR
        website LIKE '%linkedin.com%' OR website LIKE '%whatsapp.com%' OR website LIKE '%t.me%' OR
        website LIKE '%justdial.com%' OR website LIKE '%indiamart.com%' OR
        website LIKE '%tripadvisor.com%' OR website LIKE '%yelp.com%')");
    @$db->query("UPDATE lead_gen_results SET has_website=1 WHERE website IS NOT NULL AND website!='' AND has_website=0
        AND website NOT LIKE '%facebook.com%' AND website NOT LIKE '%fb.com%' AND website NOT LIKE '%instagram.com%'
        AND website NOT LIKE '%twitter.com%' AND website NOT LIKE '%x.com%' AND website NOT LIKE '%g.co%'
        AND website NOT LIKE '%goo.gl%' AND website NOT LIKE '%maps.google%' AND website NOT LIKE '%youtube.com%'
        AND website NOT LIKE '%linkedin.com%' AND website NOT LIKE '%whatsapp.com%' AND website NOT LIKE '%t.me%'
        AND website NOT LIKE '%justdial.com%' AND website NOT LIKE '%indiamart.com%'
        AND website NOT LIKE '%tripadvisor.com%' AND website NOT LIKE '%yelp.com%'");
}

// ── QUOTA SYSTEM ──────────────────────────────────────────────────────────────
// Uses 2 JSON rows in lead_gen_settings (no new tables):
//   quota_roles → {"admin":300,"manager":100,"user":30}
//   quota_users → {"5":50,"12":0}   0=blocked, -1=inherit role default
//
...
function lgUserQuota(mysqli $db, array $user, int $globalQuota): array {
    if (function_exists('isAdmin') && isAdmin()) {
        return ['quota'=>$globalQuota,'source'=>'global','blocked'=>false];
    }
    $cfg     = lgGetQuotaConfig($db);
    ...
    if (array_key_exists($uid_str,$cfg['users'])) {
        ...
        if ($v === -1) { /* fall through to role */ }
        else           return ['quota'=>$v,'source'=>'user','blocked'=>false];
    }
    // Role-based
...
if ($action === 'search') {
    ...
    $userQuota = $uqInfo['quota'];
    $usage     = lgMonthUsed($db,$uid);
    $usedLeads = (int)$usage['leads'];
    if ($usedLeads+$count > $userQuota) {
        $rem=max(0,$userQuota-$usedLeads);
        echo json_encode(['ok'=>false,'error'=>"Your monthly lead quota is $userQuota ($usedLeads used, $rem remaining). Ask admin to increase your limit."]); exit;
    }
    // Global budget guard
    ...
    if ($globalUsedCost+($cost_ts+$cost_det*$count)>$budget_usd) {
        echo json_encode(['ok'=>false,'error'=>'System API budget reached for this month. Contact admin.']); exit;
    }
    ...
    foreach ($results as $place) {
        ...
        if ($place_id) {
            ...
            if ($det_raw) {
                ...
                if (!empty($res['reviews'])) foreach ($res['reviews'] as $rev) {
                    if (!empty($rev['owner_reply']['author_name'])) { $owner_name=$rev['owner_reply']['author_name']; break; }
                }
            }
        }
        ...
        $pe=$db->real_escape_string($place_id);$ne=$db->real_escape_string($name);
        $one=$db->real_escape_string($owner_name);$phe=$db->real_escape_string($phone);
        $ee=$db->real_escape_string($email);$ae=$db->real_escape_string($address);
        ...
        $db->query("INSERT INTO lead_gen_results (user_id,place_id,name,owner_name,phone,email,address,website,has_website,rating,ratings_total,price_level,opportunity_score,location,industry,imported,api_calls)
            VALUES ($uid,'$pe','$ne','$one','$phe','$ee','$ae','$we',$has_website,$rt,$ratings_total,$pl,$opp,'$le','$ie',0,$api_calls)
            ON DUPLICATE KEY UPDATE name='$ne',owner_name='$one',phone='$phe',email='$ee',address='$ae',website='$we',has_website=$has_website,ratings_total=$ratings_total,price_level=$pl,opportunity_score=$opp");
    ...
    echo json_encode(['ok'=>true,'leads'=>$leads,'used'=>$newUsed,'quota'=>$userQuota,
        'quota_source'=>$uqInfo['source'],'remaining'=>max(0,$userQuota-$newUsed),
...
if ($action === 'get_all_stored') {
    $page=$max1=(int)($_GET['page']??1); $page=max(1,$page);
    ...
    $rows=[];
    $q=@$db->query("SELECT r.id,r.name,r.owner_name,r.phone,r.email,r.address,r.website,r.has_website,
                           r.rating,r.ratings_total,r.opportunity_score,r.location,r.industry,
                           r.imported,r.lead_id,r.created_at,r.user_id
                    FROM lead_gen_results r WHERE $whereSQL ORDER BY r.id DESC LIMIT $per_page OFFSET $offset");
...
if ($action === 'get_search_history') {
    ...
    $q=@$db->query("SELECT * FROM lead_gen_results WHERE user_id=$uid AND location='$le' AND industry='$ie'
                      AND created_at BETWEEN DATE_SUB('$created',INTERVAL 5 MINUTE) AND DATE_ADD('$created',INTERVAL 5 MINUTE) ORDER BY id ASC LIMIT 20");
...
if ($action === 'get_stats') {
    $globalQuota=(int)lgGet($db,'monthly_quota','300');
    $budget=(float)lgGet($db,'monthly_budget_usd','15.00');
    $api_set=lgGet($db,'google_api_key')!=='';
    $usage=lgMonthUsed($db,$uid);
    $uqInfo=lgUserQuota($db,$user,$globalQuota);
    $userQuota=$uqInfo['quota'];$usedLeads=(int)$usage['leads'];
    $trend=[];
    ...
    $gr=@$db->query("SELECT COALESCE(SUM(estimated_cost),0) c FROM lead_gen_usage WHERE YEAR(created_at)=YEAR(NOW()) AND MONTH(created_at)=MONTH(NOW())");
    $globalUsedCost=$gr?(float)($gr->fetch_assoc()['c']??0):0;
    echo json_encode(['ok'=>true,'used'=>$usedLeads,'quota'=>$userQuota,'quota_source'=>$uqInfo['source'],
        'remaining'=>max(0,$userQuota-$usedLeads),'blocked'=>$uqInfo['blocked'],
        'cost'=>round((float)$usage['cost'],4),'global_cost'=>round($globalUsedCost,4),'budget'=>$budget,
        'api_set'=>$api_set,'trend'=>$trend,'recent'=>$recent,'total_imp'=>$total_imp,'total_all'=>$total_all]);exit;
}

// ── GET QUOTA CONFIG (admin only) ─────────────────────────────────────────────
...
if ($action === 'get_quota_config') {
    ...
    $globalQuota=(int)lgGet($db,'monthly_quota','300');
    $users=[];
    $uq=@$db->query("SELECT u.id,u.name,u.email,u.role,COALESCE(m.leads,0) used_leads,COALESCE(m.cost,0) used_cost
                     FROM users u LEFT JOIN (
                         SELECT user_id,SUM(result_count) leads,SUM(estimated_cost) cost
                         FROM lead_gen_usage WHERE YEAR(created_at)=YEAR(NOW()) AND MONTH(created_at)=MONTH(NOW()) GROUP BY user_id
                     ) m ON m.user_id=u.id ORDER BY u.role ASC,u.name ASC");
    if ($uq) while ($row=$uq->fetch_assoc()) {
        $uid_str=(string)$row['id'];
        $eq=lgUserQuota($db,$row,$globalQuota);
        $row['effective_quota']=$eq['quota'];$row['quota_source']=$eq['source'];
        $row['user_override']=array_key_exists($uid_str,$cfg['users'])?(int)$cfg['users'][$uid_str]:null;
        $row['blocked']=$eq['blocked'];$users[]=$row;
    }
    echo json_encode(['ok'=>true,'roles'=>$cfg['roles'],'users'=>$users,'global_quota'=>$globalQuota]);exit;
}

// ── SAVE QUOTA CONFIG (admin only) ────────────────────────────────────────────
...
if ($action === 'save_quota_config') {
    ...
    $cfg=lgGetQuotaConfig($db);
    $userOverrides=$cfg['users'];
    if (!empty($_POST['user_quota'])&&is_array($_POST['user_quota'])) {
        foreach ($_POST['user_quota'] as $puid=>$pval) {
            $puid=(string)(int)$puid; $pval=trim($pval);
            if ($pval===''||$pval==='-1') unset($userOverrides[$puid]);
            else $userOverrides[$puid]=(int)$pval;
        }
    }
    lgSet($db,'quota_roles',json_encode($roles),$uid);
    lgSet($db,'quota_users',json_encode($userOverrides),$uid);
